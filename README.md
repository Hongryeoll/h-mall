...
| **경로**                                         | **렌더링 방식**   | **선택 이유 및 설명**                      |
| ------------------------------------------------ | ----------------- | ------------------------------------------ |
| /login                                           | **CSR + SSG**     | 로그인 페이지는 입력 폼 중심,              |
| SEO 중요성 낮음. 정적 빌드 후 클라이언트 렌더링. |
| /signup                                          | **CSR**           | 'use client' + Supabase 기반 인증 처리.    |
| 실시간 유효성 검사, UX 중심.                     |
| /signup/layout.tsx                               | **SSR (default)** | 단순 wrapper. 특별한 클라이언트 동작 없음. |
| 기본 서버 컴포넌트로 렌더링.                     |
